...
const SUMMARY =
  "Explore our facilities, processing lines, and logistics operations — see the scale and precision behind NRK Iron & Steel.";
const DESCRIPTION =
  "Witness the monumental scale of our manufacturing operations and stay informed with company announcements, technical updates, and industry insights from NRK Iron & Steel.";
const BADGE = "Facilities · Operations · Logistics";
...
export function GalleryHero() {
  ...
  useHeroTypewriter(heroRef, mounted, ["gallery-page"]);
  ...
  useEffect(() => {
    ...
    const typeDuration =
      250 +
      (EYEBROW.length + TITLE_PARTS.before.length + TITLE_PARTS.highlight.length + TITLE_PARTS.after.length) *
        42 +
      180 +
      200;
    ...
    >
      ...
      <div className="relative mx-auto flex min-h-[255px] max-w-container-max flex-col justify-end px-gutter pb-9 pt-20 md:min-h-[315px] md:pb-12 md:pt-24">
        ...
        <div className="max-w-3xl text-on-primary">
          ...
          >
            <span className="material-symbols-outlined text-base text-primary-fixed-dim">photo_library</span>
            {BADGE}
          </div>
        </div>
      </div>
